...
namespace Dat_ve_xp_Website
{
    public class ThuVien
    {
        ...
        public bool Ham_ExecuteNonQuery(string sql)
        {
            bool result = true;
            try
            {
        ...
        {
            int phantucuoicungcuamang = Array_Pearsion.GetLength(0) - 1;
            max1 = 0;
            for(int i = 0; i < Array_Pearsion.GetLength(0); i++)
            {

                if (Array_Pearsion[phantucuoicungcuamang, i] > Array_Pearsion[phantucuoicungcuamang, max1])
                    if (phantucuoicungcuamang != i)
                            max1 = i;
            }
            max2 = 0;
            for(int i = 0; i< Array_Pearsion.GetLength(0); i++)
            {
                ...
                if (Array_Pearsion[phantucuoicungcuamang, i] > Array_Pearsion[phantucuoicungcuamang, max2])
                    if (phantucuoicungcuamang != i && i != max1)
                        max2 = i;
            }
        }
        ...
        public double[] Goi_Y_Gia_Tri_Trung_Binh_Cho_User_Input(float[,] Array_AVG_Ratings_Item, double[,] Array_Pearsion, int max1, int max2, int index_user_id, double avg_ofuser)
        {
            ...
            for (int i = 0; i < c; i++)
            {
                if (Array_AVG_Ratings_Item[index_user_id, i] == 0)
                {
                    ...
                    result[i] = -1;//(double)Array_AVG_Ratings_Item[index_user_id, i];
            }
            return result;
        }
